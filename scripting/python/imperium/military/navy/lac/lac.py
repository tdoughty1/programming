...
class FlightCrew(Unit):
    ...
    def _SetPositions(self):
        self._AddPosition('Flight Commander', 'A', 'O3',
                          posRanks=('O2', .15))
...
class RepairTeam(Unit):
    ...
    def _SetPositions(self):
        ...
        for i in range(1, 9):
            self._AddPosition('Crewman', 'A', 'E2',
                              posRanks=(['E5', 'E4', 'E3', 'E1'],
                                        [.1, .15, .25, .2]))

    def _SetWatch(self):

        self._Watch = {'Alpha': [], 'Bravo': [], 'Charlie': [],
                       'Delta': []}

        rands = sample(range(1, len(self._CTOE[1:]) + 1),
                       len(self._CTOE[1:]))
